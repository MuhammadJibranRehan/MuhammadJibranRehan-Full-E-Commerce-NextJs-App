...
const Cart = () => {
  ...
  const updateQuantity = (id: number, newQuantity: number) => {
    setCartItems((prevItems) =>
      prevItems.map((item) =>
        item.id === id ? { ...item, quantity: Math.max(1, newQuantity) } : item // Ensuring quantity is always >= 1
      )
    );
  };
  ...
  const calculateTotal = () => {
    return cartItems.reduce(
      (total, item) => total + item.price * (item.quantity || 1), // Default to 1 if quantity is invalid
      0
    );
  };
            ...
            {cartItems.map((item) => (
              <tr key={item.id} className="text-center">
                ...
                <td className="p-4 border">
                  <input
                    type="number"
                    value={item.quantity || 1} // Default to 1 if quantity is invalid or undefined
                ...
                <td className="p-4 border">
                  ${(item.price * (item.quantity || 1)).toFixed(2)} {/* Fallback to 1 if quantity is invalid */}
                </td>
              </tr>
            ))}
          </tbody>
